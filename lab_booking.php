...
try {
    $db->query("CREATE TABLE IF NOT EXISTS lab_packages (id INT AUTO_INCREMENT PRIMARY KEY, hospital_id INT NOT NULL, name VARCHAR(255) NOT NULL, price DECIMAL(12,2) NOT NULL DEFAULT 0, sample_type VARCHAR(255) NULL, turnaround_time VARCHAR(255) NULL, description TEXT NULL, is_active TINYINT(1) NOT NULL DEFAULT 1, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, INDEX (hospital_id)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
    $db->execute();
    $db->query("CREATE TABLE IF NOT EXISTS lab_package_services (id INT AUTO_INCREMENT PRIMARY KEY, package_id INT NOT NULL, name VARCHAR(255) NOT NULL, price DECIMAL(12,2) NOT NULL DEFAULT 0, description TEXT NULL, sort_order INT NOT NULL DEFAULT 0, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, INDEX (package_id)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
    $db->execute();
        ...
        <div class="row align-items-center g-4">
            <div class="col-lg-7">
                <div class="bg-white rounded-5 shadow-sm p-4 p-md-5">
                    <h1 class="fw-bold text-uppercase mb-3" style="color:#00a8f0; font-size:clamp(2rem,4vw,3rem);">Đặt lịch xét nghiệm</h1>
                    <div class="d-flex flex-column gap-3" style="color:#023f6d; font-size:1.05rem;">
                        <div><i class="bi bi-check-circle-fill text-success me-2"></i>Đặt lịch xét nghiệm trực tiếp, không qua khâu khám trước.</div>
                        <div><i class="bi bi-check-circle-fill text-success me-2"></i>Giảm thiểu thời gian chờ đợi.</div>
                        <div><i class="bi bi-check-circle-fill text-success me-2"></i>Được hoàn phí nếu hủy phiếu đúng quy định.</div>
                        <div><i class="bi bi-check-circle-fill text-success me-2"></i>Linh hoạt lựa chọn xét nghiệm tại cơ sở hoặc tại nhà.</div>
                    </div>
            ...
            <div class="col-lg-5 text-center d-none d-lg-block">
                <img src="https://img.icons8.com/clouds/500/microscope.png" alt="Đặt lịch xét nghiệm" class="img-fluid" style="max-height:340px; object-fit:contain;">
            </div>
        </div>
    </div>
</div>
...
<div class="py-4" style="background:#eaf7ff; margin-left:calc(50% - 50vw); margin-right:calc(50% - 50vw);">
    <div class="container">
        <div class="d-flex align-items-center gap-3 mb-4">
            <form class="d-flex align-items-center gap-3 bg-white rounded-pill shadow-sm p-2 flex-fill" style="max-width: 860px;" method="get" action="lab_booking.php">
                <div class="input-group flex-fill">
                    <span class="input-group-text bg-white border-0 ps-3"><i class="bi bi-search text-muted"></i></span>
                    <input type="text" name="q" value="<?php echo htmlspecialchars($keyword); ?>" class="form-control border-0 shadow-none py-2" placeholder="Tìm kiếm dịch vụ">
                </div>
            ...
            <button type="button" class="btn btn-outline-info rounded-pill px-4 py-3 fw-bold flex-shrink-0 bg-white" data-bs-toggle="modal" data-bs-target="#labFilterModal"><i class="bi bi-sliders me-2"></i>Bộ lọc</button>
        </div>
                    ...
                    <div class="card border-0 shadow-sm rounded-4 h-100">
                        <div class="card-body p-4 d-flex gap-3 align-items-start">
                            ...
                            <div class="flex-fill">
                                ...
                                <div class="text-warning fw-bold mb-2">Giá: <?php echo number_format((float)$package['price'], 0, ',', '.'); ?>đ</div>
                                <div class="small text-muted mb-3"><?php echo htmlspecialchars($package['turnaround_time'] ?: 'Thời gian trả kết quả đang cập nhật'); ?></div>
                                <?php $services = array_filter(explode('||', $package['service_names'] ?? '')); ?>
                                <?php if (count($services) > 0): ?>
                                    <div class="small mb-3" style="color:#023f6d;">
                                        <?php foreach ($services as $service): ?><div>• <?php echo htmlspecialchars($service); ?></div><?php endforeach; ?>
                                    </div>
                                <?php endif; ?>
...
<div class="modal fade" id="labFilterModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content border-0 rounded-4 overflow-hidden">
            ...
            <div class="modal-body p-4">
                <div class="input-group rounded-pill border mb-4 mx-auto" style="max-width:470px;">
                    <span class="input-group-text bg-white border-0 ps-4"><i class="bi bi-search text-muted"></i></span>
                    <input type="text" id="provinceSearch" class="form-control border-0 shadow-none py-2" placeholder="Tìm kiếm">
                </div>
                ...
                <div class="d-flex flex-wrap gap-2" id="provinceList">
                    <?php foreach ($provinces as $index => $item): ?>
                        <a href="lab_booking.php?province=<?php echo urlencode($item); ?><?php echo $keyword !== '' ? '&q=' . urlencode($keyword) : ''; ?>" class="province-chip btn rounded-pill px-3 py-2 <?php echo $province === $item ? 'btn-info text-white' : 'btn-outline-info'; ?> <?php echo $index >= 8 ? 'd-none extra-province' : ''; ?>" data-name="<?php echo htmlspecialchars(mb_strtolower($item, 'UTF-8')); ?>">
            ...
            <div class="modal-footer border-0" style="background:#eaf7ff;">
                <a href="lab_booking.php" class="btn btn-outline-info rounded-pill px-4">Đặt lại</a>
                <button type="button" class="btn btn-premium-primary rounded-pill px-4" data-bs-dismiss="modal">Áp dụng</button>
            </div>
        </div>
    </div>
</div>
...
document.getElementById('provinceSearch')?.addEventListener('input', function () {
    ...
    document.querySelectorAll('.province-chip').forEach(function (item) {
        item.classList.toggle('d-none', keyword && !item.dataset.name.includes(keyword));
    });
});
</script>
